    ...
    def handle(self, *args, **options):
        while True:
            for conf in Config.objects.all():
                if conf.sent_timestamp and ((timezone.localtime() - conf.sent_timestamp).total_seconds() / 60) < conf.minutes:
                    continue
                ...
                qs = Odd.objects.filter(house__in=houses,
                                        ...
                                        date__gte=timezone.localtime().date(),
                                        date__lte=(timezone.localtime() + timedelta(days=1)).date()).order_by('date', 'hour')
                ...
                for key, odds in odds.items():
                    ...
                    message_map = {
                        ...
                        'mercado_casa1': escape_markdown(odds[0].market.split('title="', 1)[1].split('"', 1)[0].strip(), 2),
                        'odd_casa1': escape_markdown(str(round(odds[0].chance, 2)), 2),
                        'link_casa1': escape_markdown(f'https://sinais.sempregreen.com.br/redirect/{odds[0].surebet_id}/{odds[0].house}', 2),
                        'mercado_casa2': escape_markdown(odds[1].market.split('title="', 1)[1].split('"', 1)[0].strip(), 2),
                        'casa2': escape_markdown(odds[1].house, 2),
                        'odd_casa2': escape_markdown(str(round(odds[1].chance, 2)), 2),
                        'link_casa2': escape_markdown(f'https://sinais.sempregreen.com.br/redirect/{odds[1].surebet_id}/{odds[1].house}', 2),
                        'link_calculadora': escape_markdown(f'https://sinais.sempregreen.com.br/calc/{odds[1].surebet_id}/{odds[0].house}/{odds[1].house}', 2),
                    }
