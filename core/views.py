...
def relatorio_view(request):
    if request.method == 'POST':
        ...
        total_vendas = Payment.objects.filter(modified__range=(data_inicio, data_fim)).count()
        total_vendas_pagas = Payment.objects.filter(status='APPROVED', modified__range=(data_inicio, data_fim)).count()
        total_vendas_canceladas = Payment.objects.filter(status='EXPIRED', modified__range=(data_inicio, data_fim)).count()
        total_vendas_pendentes = Payment.objects.filter(status='PENDING', modified__range=(data_inicio, data_fim)).count()

        valor_pedidos = Payment.objects.filter(modified__range=(data_inicio, data_fim)).aggregate(Sum('plan__amount'))
        valor_pedidos_pagos = Payment.objects.filter(status='APPROVED', modified__range=(data_inicio, data_fim)).aggregate(Sum('plan__amount'))
        valor_pedidos_cancelados = Payment.objects.filter(status='EXPIRED', modified__range=(data_inicio, data_fim)).aggregate(Sum('plan__amount'))
        valor_pedidos_pendentes = Payment.objects.filter(status='PENDING', modified__range=(data_inicio, data_fim)).aggregate(Sum('plan__amount'))

        clientes_total = User.objects.filter(date_joined__range=(data_inicio, data_fim)).count()
        clientes_ativos = User.objects.filter(plan_is_active=True, date_joined__range=(data_inicio, data_fim)).count()
        clientes_inativos = User.objects.filter(plan_is_active=False, plan_id__isnull=False, date_joined__range=(data_inicio, data_fim)).count()
        clientes_sem_plano = User.objects.filter(plan_is_active=False, plan_id__isnull=True, date_joined__range=(data_inicio, data_fim)).count()

        clientes_por_plano = User.objects.filter(date_joined__range=(data_inicio, data_fim)).values('plan__name').annotate(count=Count('plan'),sum_amount=Sum('plan__amount')).order_by('-count')
...
class RegisterView(TemplateView):
    ...
    def post(self, request):
        ...
        user = User.objects.create(
            ...
            produto_link=form.cleaned_data['produto_link'],
            produto2_link=form.cleaned_data['produto2_link'],
            produto3_link=form.cleaned_data['produto3_link'],
            telsuporte=form.cleaned_data['telsuporte'],
            username=form.cleaned_data['email'],
            plan_id=form.cleaned_data['plan_id'],
            plan_is_active=True,
            plan_expire=form.cleaned_data['plan_expire'],
            is_active=True,
            referer=referer
        )
...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        live_houses = 'Vbet,leovegas,Betano,Pinnacle,Netbet,KTO,Sportingbet,Bodog,Betway,Betsson,bet365,188bet,Betfair,Novibet'.split(',')
...
class GerenciamentoView(TemplateView):
    template_name = 'gerenciamento.php'
        ...
        if 'betano' in odd.house.lower():
            ...
            for request in driver.requests:
                if 'www.vbet.com/' in request.url and 'eventId' in request.url:
                    redirect_uri = request.url
                    break
        elif 'pinnacle' in odd.house.lower():
            for request in driver.requests:
                if 'pinnacle.com/' in request.url:
                    redirect_uri = request.url
                    break
        elif 'betmotion' in odd.house.lower():
            for request in driver.requests:
...
@method_decorator([login_required], name='dispatch')
class CalcTesteView(TemplateView):
    template_name = 'calcteste.html'
...
class ProcessPayment(View):
    def post(self, request):
        ...
        if 'apostariscozero' in request.get_host():
            sdk = mercadopago.SDK('APP_USR-8373317421326537-112923-3b8879942720ecb0dce9e91c264ddd35-528935715')
        else:
            sdk = mercadopago.SDK('APP_USR-8373317421326537-112923-3b8879942720ecb0dce9e91c264ddd35-528935715')
        ...
        # Renderizando o código do pixel do Facebook
        pixel_code = '''
        <!-- Meta Pixel Code -->
        <script>
        ...
        fbq('init', '559276123079682');
        fbq('track', 'PageView');
        </script>
        <noscript><img height="1" width="1" style="display:none"
        src="https://www.facebook.com/tr?id=559276123079682&ev=PageView&noscript=1"
        /></noscript>
        <!-- End Meta Pixel Code -->
        '''

        # Redirecionando para a URL do ticket e incluindo o código do pixel na resposta HTML
        ticket_url = payment_response["response"]['point_of_interaction']['transaction_data']['ticket_url']
        response = HttpResponse(pixel_code + f'<script>location.href="{ticket_url}";</script>')
        return response
...
class ProcessPaymentCard(View):
    def post(self, request):
        ...
        if 'apostariscozero' in request.get_host():
            ...
            notification_uri = 'https://sinais.sempregreen.com.br' + reverse_lazy('notification')
            sdk = mercadopago.SDK('APP_USR-8373317421326537-112923-3b8879942720ecb0dce9e91c264ddd35-528935715')
        ...
        # Renderizando o código do pixel do Facebook na resposta HTTP
        pixel_code = '''
        ...
        if status == 'approved':
            ...
            # Redirecionar para o link após o pagamento aprovado
            return redirect('https://sempregreen.com.br/obrigado-compra/')

        # return HttpResponse('', status=201)

        return HttpResponse(pixel_code, status=201)
...
class ReferersView(View):
    def get(self, request, email):
        ...
        return redirect(reverse_lazy('addcliente'))
...
class PaymentNotification(View):
    def post(self, request):
        ...
        if 'apostariscozero' in request.get_host():
            token = 'APP_USR-8373317421326537-112923-3b8879942720ecb0dce9e91c264ddd35-528935715'
        else:
            token = 'APP_USR-8373317421326537-112923-3b8879942720ecb0dce9e91c264ddd35-528935715'
